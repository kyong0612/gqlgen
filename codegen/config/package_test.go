...
func TestPackageConfig(t *testing.T) {
	t.Run("when given just a filename", func(t *testing.T) {
		...
		require.Equal(t, "github.com/kyong0612/gqlgen/codegen/config/testdata", p.ImportPath())

		require.Equal(t, "config_test_data", p.Pkg().Name())
		require.Equal(t, "github.com/kyong0612/gqlgen/codegen/config/testdata", p.Pkg().Path())
